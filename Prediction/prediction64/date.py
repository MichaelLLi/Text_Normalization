            ...
            if len(seg) == 1:
                ...
                if (len(segfors) == 1):
                    if (str(segfors[0])[0] in numbers):
                        if (int(seg[0]) <= 1000) | (int(seg[0]) >= 2100):
                            output[j] = num2words(int(seg[0]))
                        ...
                        if str(seg[1]) in month_sim:
                            if (int(str(seg[0])) <= 31):
                                if(str(seg[2])[0] in numbers):
                                    if len(str(seg[2])) == 2:
                                        if str(seg[2])[0] == '0':
                                            output[j] = 'the' + ' ' + num2words(int(str(seg[0])),ordinal = True) + ' ' + 'of' + ' ' + month_sim[str(seg[1])]+ ' o ' + dateyear(seg[2][-1])
                                            output[j] = re.sub('-',' ',output[j])
                                        else:
